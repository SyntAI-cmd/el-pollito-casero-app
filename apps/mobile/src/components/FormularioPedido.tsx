...
import { Vacio } from '@/components/ui/Pantalla';
import { Tarjeta } from '@/components/ui/Tarjeta';
import { Texto } from '@/components/ui/Texto';
import { api, desenvolver, mensajeDeError, type Cliente, type Esquemas } from '@/lib/api';
import { useClientes, usePreciosCliente, useProductos, useUsuarios } from '@/lib/consultas';
...
/**
 * Cargar pedido: cliente, cajas o kilos por producto, precio editable en la fila (con opción de
 * guardarlo como propio), turno, cuenta corriente y observaciones. Administración además elige el
 * preventista y el segundo preventista.
 */
export function FormularioPedido({
  onCreado,
  fecha,
}: {
  onCreado: (pedido: Esquemas['PedidoSalida']) => void;
  fecha?: string;
}) {
  const queryClient = useQueryClient();
  const esAdmin = useSesion((s) => s.usuario?.rol === 'admin');
  ...
  const [segundoId, setSegundoId] = useState<string | null>(null);
      ...
      desenvolver(
        await api.POST('/pedidos', {
          body: {
            ...
            fecha_reparto: fecha ?? hoyIso(),
            turno,
            a_cuenta: aCuenta,
            observaciones,
            items,
            preventista_id: esAdmin ? preventistaId : null,
            segundo_preventista_id: esAdmin ? segundoId : null,
          },
        }),
      ),
    onSuccess: (pedido) => {
            ...
            {esAdmin ? (
              <View className="mt-3 gap-2">
                ...
                <View className="flex-row flex-wrap gap-2" accessibilityRole="radiogroup">
                  {preventistas.map((u) => (
                    <Pressable
                      key={u.id}
                      accessibilityRole="radio"
                      accessibilityState={{ checked: preventistaId === u.id }}
                      onPress={() => setPreventistaId(preventistaId === u.id ? null : u.id)}
                      className={`min-h-[48px] justify-center rounded-pill border px-4 ${preventistaId === u.id ? 'border-primary bg-primary' : 'border-border bg-surface'}`}>
                      <Texto variante="label-md" tono={preventistaId === u.id ? 'claro' : 'normal'}>
                        {u.nombre}
                      </Texto>
                    </Pressable>
                  ))}
                ...
                {preventistaId ? (
                  <>
                    <Texto variante="label-md" tono="suave">
                      Segundo preventista (van dos por camión)
                    </Texto>
                    <View className="flex-row flex-wrap gap-2" accessibilityRole="radiogroup">
                      {preventistas
                        .filter((u) => u.id !== preventistaId)
                        .map((u) => (
                          <Pressable
                            key={u.id}
                            accessibilityRole="radio"
                            accessibilityState={{ checked: segundoId === u.id }}
                            onPress={() => setSegundoId(segundoId === u.id ? null : u.id)}
                            className={`min-h-[48px] justify-center rounded-pill border px-4 ${segundoId === u.id ? 'border-charcoal bg-charcoal' : 'border-border bg-surface'}`}>
                            <Texto
                              variante="label-md"
                              tono={segundoId === u.id ? 'claro' : 'normal'}>
                              {u.nombre}
                            </Texto>
                          </Pressable>
                        ))}
              ...
              <Tarjeta key={prod.codigo}>
                <View className="flex-row items-center justify-between">
                  ...
                  <Pressable
                    ...
                    {precioMostrado ? (
                      <Texto variante="body-metric" tono={r.precio.trim() ? 'primario' : 'normal'}>
                        {precioMostrado}/kg
                      </Texto>
                    ) : (
                      <Badge estado="sin_precio" texto="Sin precio" />
                    )}
                    <MaterialCommunityIcons name="pencil" size={18} color={tokens.colors.primary} />
                  </Pressable>
                </View>
